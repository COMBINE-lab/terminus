...
fn do_group(sub_m: & ArgMatches) -> Result<bool, io::Error> {
    ...
    let mut gibbs_array = Array2::<f64>::zeros((x.num_valid_targets as usize, x.num_bootstraps as usize));
    ...
    let min_spread = sub_m.value_of("min-spread").unwrap().parse::<f64>()
                     .expect("could not convert min-spread to float value");

    let tolerance = sub_m.value_of("tolerance").unwrap().parse::<f64>()
                    .expect("could not convert tolerance to float value"); 

    let mut dfile = File::create(file_list_out.delta_file.clone()).expect("could not create collapse.log");
    let mut unionfind_struct = UnionFind::new(eq_class.ntarget);
    let mut gr = util::eq_experiment_to_graph(&eq_class, &mut gibbs_array, &eq_class_counts, tolerance, thr, p, min_spread, &mut dfile, &mut unionfind_struct);
    ...
    let mut cfile = File::create(file_list_out.collapsed_log_file.clone()).expect("could not create collapse.log");

    let gcomp : Vec<petgraph::prelude::NodeIndex> = gr.node_indices().map(|x| petgraph::graph::NodeIndex::new(x.index())).collect();
    util::work_on_component(&eq_class_counts, &mut gibbs_array, 
        &mut gibbs_mat_mean, &mut unionfind_struct, &mut gr, 
        &gcomp, &mut num_collapses, thr, p, &mut cfile);
    ...
    println!("Number of collapses {}", num_collapses.to_formatted_string(&Locale::en));
    //let _res = util::write_modified_quants(&groups, &grouped_set, &file_list_out, &gibbs_array, &x, &rec, &collapsed_dim);
    let mut gfile = File::create(file_list_out.group_file.clone()).expect("could not create groups.txt");
...
fn do_collapse(sub_m: & ArgMatches) -> Result<bool, io::Error> {
    ...
    let num_threads = sub_m.value_of("threads").unwrap().parse::<usize>().expect("could not parse --threads option");
    ...
    // filter
    let consensus_thresh = sub_m.value_of("consensus-thresh").unwrap().parse::<f64>().expect("could not parse --consensus-thresh option"); 
    ...
    let (total_in_group, num_group) = comps.iter().fold(
        (0, 0), |s, v| { if v.len() > 1 { (s.0 + v.len(), s.1+1) } else { (s.0, s.1) } });
    let term_info = salmon_types::TerminusInfo{ 
        num_nontrivial_groups : num_group, 
        num_targets_in_nontrivial_group : total_in_group 
    };
    println!("total number of transcripts in a non-trivial group : {}", total_in_group);
    ...
    dir_paths.into_par_iter().for_each(|dname| {
        ...
        ::serde_json::to_writer( &File::create(term_info_path).expect(&estr), &term_info).expect(&estr);
        // load old files
        // let x = util::parse_json(&file_list.mi_file).unwrap();
        // copy cmd_info.json from old location to new
        std::fs::copy(&file_list.cmd_file, &file_list_out.cmd_file).expect("Could not copy cmd_info.json.");
        let x = util::parse_json(&file_list.mi_file).unwrap();
        ...
        //call the writer
        let _res = util::write_quants_from_components(
            &comps,
            &file_list_out,
            &gibbs_array,
            &x,
