...
fn get_merged_bparts(groups:&HashMap<usize,Vec<usize>>, 
    all_groups_bpart:&HashMap<String, BTreeMap<String, u32>>, 
    uf:&UnionFind<usize>) -> HashMap<String, BTreeMap<String, u32>> {
    let all_groups:Vec<String> = all_groups_bpart.keys().cloned().collect();
    let mut merged_bparts:HashMap<String, BTreeMap<String,u32>> = HashMap::new();
    for (j, old_g) in all_groups.iter().enumerate(){
        ...
        let m_bpart_key = merged_bparts.entry(sort_group_id(&m_group.clone())).or_insert(BTreeMap::new());
...
pub fn merge_groups(all_groups_bpart:&HashMap<String, BTreeMap<String, u32>>, 
                    ntxps:usize) -> HashMap<String, BTreeMap<String, u32>> {
